...
func fastlz2Compress(input, output []byte) int {
	...
	for len(ip) >= len(ipLimit) {
		...
		op = flz2Match(uint32(length), distance, op)
...
func flz2Match(length, distance uint32, op []byte) []byte {
	distance--
	if distance < MaxL2Distance {
		if length < 7 {
			...
			op = append(op, byte(length))
			op = append(op, byte(distance&255))
		}
	} else {
		// Far away, but not yet in another galaxy...
		if length < 7 {
			distance -= MaxL2Distance
			op = append(op, byte((length<<5)+31))
			op = append(op, 255)
			op = append(op, byte(distance>>8))
			op = append(op, byte(distance&255))
		} else {
			distance -= MaxL2Distance
			op = append(op, byte((7<<5)+31))
			for length -= 7; length >= 255; length -= 255 {
				op = append(op, 255)
			}
			op = append(op, byte(length))
